/////////////////////////////////////////////////////////////////
//   This is MMS64LP-hi random number generator at 2026.04.11. //
//     ([Code-Name:'KUNIKOsan-20260411hi']).                   //
//   Use the compiler gcc.                                     //
//   MMS64LP-hi is an academic product.                        //
//   Use MMS64LP-hi within your responsibility.                //
/////////////////////////////////////////////////////////////////
...
#ifndef originalMMS
unsigned long long int MMS64LP_getBin64_org(void)  //return 64-bit RN
    {
...
unsigned long long int MMS64LP_getBin64(void)  //return 64-bit RN
    ...
    uiLLHL  XLL3, XLL4;
    my_i128 u;
    XLL1.I = SIParaMMS64LP.XpBase;      // B(base)
    XLL1.I ^= (SIParaMMS64LP.rk);       // X_i = (B XOR P_i) in (S0)

    XLL2.I = SIParaMMS64LP.XpBase;      // B(base)
    XLL2.I ^= (SIParaMMS64LP.sk);       // Y_i = (B XOR Q_i) in (S0)

   // (M_i)*(M_i) in Step (S3) is modified as
   //
   //   (M_i XOR (PP_i))*(M_i XOR (QQ)_i)

    XLL3.I = (SIParaMMS64LP.rrk);       // (PP)_i : XOR is taken with M_i in (S3)

    XLL4.I = (SIParaMMS64LP.ssk);       // (QQ)_i : XOR is taken with M_i in (S3)
    ...
    //MMS64plusRS_LP(&SIParaMMS64LP);      // prepare next P_i, ..., (QQ)_i
    SIParaMMS64LP.rk += SIParaMMS64LP.r;    // P_(i+1) = (P_i)+r mod p
    if (SIParaMMS64LP.rk >= SIParaMMS64LP.p) {SIParaMMS64LP.rk -= SIParaMMS64LP.p;}
    SIParaMMS64LP.sk += SIParaMMS64LP.s;    // Q_(i+1) = (Q_i)+s mod q
    if (SIParaMMS64LP.sk >= SIParaMMS64LP.q) {SIParaMMS64LP.sk -= SIParaMMS64LP.q;}

    SIParaMMS64LP.rrk += SIParaMMS64LP.rr;  // (PP)_(i+1) = (PP_i)+rr mod pp
    if (SIParaMMS64LP.rrk >= SIParaMMS64LP.pp) {SIParaMMS64LP.rrk -= SIParaMMS64LP.pp;}
    SIParaMMS64LP.ssk += SIParaMMS64LP.ss;  // (QQ)_(i+1) = (QQ_i)+ss mod qq
    if (SIParaMMS64LP.ssk >= SIParaMMS64LP.qq) {SIParaMMS64LP.ssk -= SIParaMMS64LP.qq;}
...
/**************/
#ifndef originalMMS
double MMS64LP_getU01_org(void)  //return RN in [0,1)
    {
    uiLLHL_Dbl MMS64Dbl;
...
//   KUNIKOsan-20260411hi) in the subject of the mail.)
//
// MMS64LP-hi RNG : Last updated at 2026.04.11
//      (Code Name : KUNIKOsan-20260411hi)
// This NOTE : Last updated at 2026.04.11
/////////////////////////////////////////////////////////
