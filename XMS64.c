//////////////////////////////////////////////////////////////
//   This is XMS64 random number generator at 2025.09.23    //
//   Use the complier gcc.                                  //
//   XMS64 is an academic product.                          //
//   Use XMS64 within your responsibility.                  //
//////////////////////////////////////////////////////////////
// Period is  (p=2^63-25)x(q=2^63-165)

/*** Remove // in the next line to test this code ***/

//#define HowToUseXMS64


#include <stdio.h>
#include <memory.h>

#define RAND_name "XMS64"
...
/**************/
void XMS64plusRS(SIPara64 *SIParaL)
    {
...
//*********************************//
unsigned long long int XMS64Ksc_A_XMS(SIPara64 *SIParaL) 
    {
    int    i;
...
/*================================================================*/
/**************/
void XMS64IniValidityCheck(mIntD *UD, mInt *Vcl, mIntD *QD, mIntD *RD, char ChkId)
    {
    mInt   U;
    mIntD  UVD, wkD;
    int    i, err=0;
    ...
    if (err != 0)
        {
        printf("Validity-Error detected by XMS64IniValidityCheck() [%ck, err#=%d]\n", ChkId, err);
        printf("\n[UD] ");
...
/**************/
void XMS64IniCom(SIPara64 *SIParaL, mInt *N)
     {
     mInt  U, V, Vcl;
     mIntD UVD, QD, RD;
     int   c, mIntResult;
     ...
     XMS64IniValidityCheck(&UVD, &Vcl, &QD, &RD, 'r');
     ...
     XMS64IniValidityCheck(&UVD, &Vcl, &QD, &RD, 's');
     LLHLwk.HL.H = RD.mIdgt[mIntIndexMax*2-1];
     LLHLwk.HL.L = RD.mIdgt[mIntIndexMax*2];
     SIParaL->sk = LLHLwk.I;
     }
...
/**************/
void XMS64Ini(unsigned long long int n)
     {
     uiLLHL LLHLn;
     mInt  U;
     ...
     XMS64IniCom(&SIParaXMS64K, &U);
     }

/**************/
void XMS64IniNM(unsigned long long int n, 
...
/**************/
void XMS64IniNML(unsigned long long int n, 
...
/*******************************************************/
/**************/
unsigned long long int XMS64_Bin64(void)
    {
    unsigned long long int RanBits;
    RanBits=XMS64Ksc_A_XMS(&SIParaXMS64K);
    XMS64plusRS(&SIParaXMS64K);
    return(RanBits);
    }
/**************/
double XMS64_U01(void)
    {
    uiLLHL_Dbl XMS64Dbl;
    XMS64Dbl.I=XMS64Ksc_A_XMS(&SIParaXMS64K);
    XMS64plusRS(&SIParaXMS64K);
    XMS64Dbl.I >>= 12;
    XMS64Dbl.I |= 0x3ff0000000000000;     /*set exponent 2^0*/
    return(XMS64Dbl.Dbl-1);
    }

/*********** How to use XMS64 **************************/
int mainSample_XMS(void)
    {
    ...
    /****************** XMS64 *******************/
    printf("======== using XMS64_Bin64() ========\n");
    printf("XMS64Ini(0)  // set to generate 1st RN\n");
    XMS64Ini(0);
    ranBin64=XMS64_Bin64();                 //the 1st random number
    printf("XMS64_Bin64()  %016llx  (= 2549db8013fc8303)  // 1st RN\n", ranBin64);
    ranBin64=XMS64_Bin64();                 //the 2nd random number
    printf("XMS64_Bin64()  %016llx  (= 8b4b011c79312836)  // 2nd RN\n", ranBin64);
    ranBin64=XMS64_Bin64();                 //the 3rd random number
    printf("XMS64_Bin64()  %016llx  (= e1a2d4f0c2625b32)  // 3rd RN\n", ranBin64);
    ranBin64=XMS64_Bin64();                 //the 4th random number
    printf("XMS64_Bin64()  %016llx  (= 8d9e8f81819305cb)  // 4th RN\n\n", ranBin64);

	printf("XMS64Ini(202)  // set to generate 202-th RN\n");
	XMS64Ini(202);
    ranBin64=XMS64_Bin64(); 
    printf("XMS64_Bin64()  %016llx  (= e619c76faf442894)  // 202_th RN\n", ranBin64);

	printf("\nXMS64IniNM(101,2)  // set to generate N*M_th RN\n");
	XMS64IniNM(101,2);
    ranBin64=XMS64_Bin64(); 
    printf("XMS64_Bin64()  %016llx  (= e619c76faf442894)  // 101*2_th RN\n", ranBin64);

	printf("\nXMS64IniNML(50,4,2)  // set to generate N*M+L_th RN\n");
	XMS64IniNML(50,4,2);
    ranBin64=XMS64_Bin64(); 
    printf("XMS64_Bin64()  %016llx  (= e619c76faf442894)  // 50*4+2_th RN\n", ranBin64);
  ...
  ////////////////////////
    ...
    printf("XMS64Ini(0)  // set to generate 1st RN\n");
    XMS64Ini(0);
    ranU01=XMS64_U01();                 //the 1st random number
    printf("XMS64_U01()  %12.10f  (= 0.1456582248)  // 1st RN\n", ranU01);
    ranU01=XMS64_U01();                 //the 2nd random number
    printf("XMS64_U01()  %12.10f  (= 0.5441132254)  // 2nd RN\n", ranU01);
    ranU01=XMS64_U01();                 //the 3rd random number
    printf("XMS64_U01()  %12.10f  (= 0.8813908661)  // 3rd RN\n", ranU01);
    ranU01=XMS64_U01();                 //the 4th random number
    printf("XMS64_U01()  %12.10f  (= 0.5532006923)  // 4th RN\n\n", ranU01);
